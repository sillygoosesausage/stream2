...
## Phase 7 — The recursive experiment (high risk, high reward)

This is the attempt to recover the ~11 RMSE points that PM2.5 history is worth (Fact 3).

**The idea:** the test block is contiguous. Your prediction for station *s* at hour *t* IS the
"current PM2.5" for station *s* at hour *t+1*. Chain them: predict the first hour (whose true
current PM2.5 is known from the last training row), feed the prediction forward, repeat.

**The danger:** ~4,255 sequential steps per station. Errors compound. A model that leans heavily on
`pm25_now` will, after a few hundred steps, be predicting mostly from its own hallucinations.
